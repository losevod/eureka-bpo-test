...
На ветке Main приложение запускается через server.bat. Или через консоль(команда: "node ./server.js").

На ветке develop нерабочая версия проекта - попытка реализовать node-addon-api приложение. Приложение работало как тестовое(Выводило в консоль "HelloWorld"). По тексту задания сделал вывод, что подходит метод NetUserEnum(https://learn.microsoft.com/en-us/windows/win32/api/lmaccess/nf-lmaccess-netuserenum), но реализовать его так и не получилось. 
